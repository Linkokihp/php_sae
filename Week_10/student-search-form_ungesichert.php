...
$hasError = false; 
$conn = mysqli_connect( 'localhost', 'root', '', 'testdatenbank' );
...
if( isset($_GET['search']) ){
	...
	if($hasError == false){
		...
		$prepared = mysqli_prepare($conn, $query); //Bereitet den den Server auf seine Aufgabe vor 
		mysqli_stmt_bind_param($prepared, 's', $_GET['search']);
		mysqli_stmt_execute($prepared); //ausführen

		$result = mysqli_stmt_get_result($prepared); //Resultat des Befehls
